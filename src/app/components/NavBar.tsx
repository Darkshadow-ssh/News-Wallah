    ...
    <header className="absolute inset-x-0 top-0 z-50">
      <nav aria-label="Global" className="flex items-center justify-between p-2 lg:px-8">
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          ...
          {navigation.map((item) => (
            <Link key={item.name} href={item.href} className="text-sm/6 font-semibold text-gray-900 hover:text-indigo-600 transition-colors dark:text-white dark:hover:text-indigo-400">
              {item.name}
            </Link>
          ))}
        </div>
        ...
        <div className="hidden lg:flex lg:flex-1 lg:justify-end lg:items-center lg:gap-x-4">
          ...
          {status === "loading" ? (
            ...
            <Popover className="relative">
              <PopoverButton className="inline-flex items-center gap-x-2 text-sm/6 font-semibold text-gray-900 hover:text-indigo-600 transition-colors dark:text-white dark:hover:text-indigo-400 outline-none">
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-500/10 dark:divide-white/10">
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-gray-900 hover:bg-gray-50 transition-colors dark:text-white dark:hover:bg-white/5"
                  >
                    {item.name}
                  </Link>
                ))}
              ...
              <div className="py-6 space-y-2">
                ...
                {status === "loading" ? (
                  <div className="-mx-3 block rounded-lg px-3 py-2.5 text-base/7 text-gray-500 dark:text-gray-400">
                    Loading...
                  </div>
                ) : session?.user ? (
                  <>
                    <div className="-mx-3 rounded-lg px-3 py-2 border-t border-gray-200 dark:border-gray-700">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {session.user.name}
                      </p>
                    ...
                    <button
                      onClick={handleSignOut}
                      className="-mx-3 flex w-full items-center gap-x-3 rounded-lg px-3 py-2.5 text-base/7 font-semibold text-gray-900 hover:bg-gray-50 transition-colors dark:text-white dark:hover:bg-white/5"
                    >
                      <ArrowRightOnRectangleIcon className="h-5 w-5" />
                      Sign out
                    </button>
                  ...
                  <Link
                    href="/auth/login"
                    className="-mx-3 block rounded-lg px-3 py-2.5 text-base/7 font-semibold text-gray-900 hover:bg-gray-50 transition-colors dark:text-white dark:hover:bg-white/5"
                  >
                    Log in
                  </Link>
                )}
